...
import { SendCoins, SendPayment } from "./node";
...
const handlerMap: HandlerMap = {
  ...
  onchain_payment_sended: async ({ address, amount }) => {
    const response = await SendCoins(address, amount);
    ...
    if (!response) {
      throw {
        ...
        errors: [
          {
            message: CustomMessage.somethingWentWrong,
            path: "sendOnchainPayment",
          },
        ],
      };
    }
    console.log("onchain_payment_sended ", response.txid);
  },
};
  ...
  const mqConsume = <Key extends keyof IncomingMessageDataMap>(
    ...
    channel.consume(queue, async (e) => {
      ...
      if (m) {
        data = JSON.parse(m);
        if (data) {
          handlerMap[data.operation](data.data as any, conn);
        }
      }
    });
  };
