import { decode as boltDecode } from "bolt11";

const bech32CharValues: string = "qpzry9x8gf2tvdw0s3jn54khce6mua7l";
...
function bech32ToInt(str: string) {
  let sum = 0;
...
function bech32ToFiveBitArray(str: string) {
  let array: number[] = [];
  for (let i = 0; i < str.length; i++) {
...
export function decode(paymentRequest) {
  ...
  return {
    human_readable_part: decodeHumanReadablePart(
      humanReadablePart,
      paymentRequest
    ),
...
function decodeHumanReadablePart(humanReadablePart, paymentRequest) {
  ...
  let amount = boltDecode(paymentRequest).satoshis;
  return {
    prefix: prefix,
    satoshis: amount,
  };
...
function decodeData(data, humanReadablePart) {
  ...
  let value: any = bech32ToFiveBitArray(date32 + tagData);
...
function decodeAmount(str) {
  ...
  let multiplier = isDigit(str.charAt(str.length - 1))
    ? "-"
...
function decodeTag(type, length, data) {
  switch (type) {
    ...
    default:
      ...
      console.log("Unknown field decoded");
      return;
  }
  return;
}
