...
import { ACHClass, BankTransferNetwork, BankTransferType } from "plaid";
...
class DepositResolver {
  ...
  async deposit(
    ...
    if (exchangeRate * amount <= myBankAccount.balance.currentBalance) {
      return {
        success: false,
        errors: [
          {
    ...
    const bfMigrateAccountResponse = await dataSources.plaidClient.bankTransferMigrateAccount(
      {
    ...
    const bfCreateResponse = await dataSources.plaidClient.bankTransferCreate({
      access_token,
